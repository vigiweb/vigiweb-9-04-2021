...
<%@page import="controlador.tipoMovDAO"%>
...
<html>
    <head>
        ...
        <script src="../js/validarTipoMov.js"></script>
    ...
    <body style="background-image: url('../imagenes/ropaRosada (1).jpg')" onload="visualizaOculta('<%=request.getParameter("Vista")%>');">
        ...
        <div id="Registrar" class="bd-example d-none">
            <div class="container" style="background-color: white">
                ...
                <form action="/vigiwebB/registrarTipoMov" method="post" onsubmit="return validarTipoMov()" >
                    <div class="form-row" >
                        <div class="form-group col-md-12" >
                            <h2>Tipo Movimiento:</h2>
                            <input type="text" name="m_descrip" class="form-control" placeholder="Digite el tipo de movimiento" id="txt_m_descrip_Reg">
                        </div> 
                    </div>
        ...
        <div id="Listado" class="bd-example contenedor2">
            <form action="/vigiwebB/GestionTipoMov" method="post">
                <div class="row justify-content-md-center">
                    ...
                    <div>
                        <h2 class="title-table">Listado Tipo Movimientos</h2>
                        <%
                            tipoMovDAO tmdao = new tipoMovDAO();
                            ArrayList<tipo_mov> tmList = new ArrayList<tipo_mov>();
                            tmList = tmdao.ConsultarListadoTipoMov(' ', buscando);
                            out.println("<table class='table table-light table-striped table-hover table-borderless border-dark'><thead class='thead-info'><tr><th>ID</th><th>Tipo Movimiento</th><th>Eliminar</th></tr></thead>");

                            for (tipo_mov C : tmList) {

                                out.println("<tr>");
                                out.println("<td>" + C.getId_tipo_mov() + "</td>");
                                out.println("<td>" + C.getM_descrip() + "</td>");

                                out.println("<td>" + "<input type = 'submit' class='btn btn-danger btn btn-login' value='Eliminar'name='Eliminar'onclick='SetIdConsulta(" + C.getId_tipo_mov() + ")'/>" + "</td>");
                                out.println("</tr>");
                            }
                            out.println("</table>");
                        %>
